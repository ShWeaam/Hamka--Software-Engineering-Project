...
public class QuestionsManagmentController implements Initializable {
	...
	void addQuestion(ActionEvent event) {
		...
		try {
			if (newAns1.getText().equals("") || newAns2.getText().equals("") || newAns3.getText().equals("") || newAns4.getText().equals("")
					|| checkDoubleAnswer(newAns1.getText(),newAns2.getText(),newAns3.getText(),newAns4.getText()) )
			...
			//TODO add questions in Frame problem
			Question questionToAdd = new Question(newContent.getText(), QuestionLevel.valueOf(newLevel.getValue().toString()), 
						answers, newCorrectAnswer.getValue() + "","Chimps");
			...
			Alerts.warning("missing or duplicate fields , please make sure all required fields are correct");

		}

	}

	private boolean checkDoubleAnswer(String ans1, String ans2, String ans3, String ans4) {
	...
	public void fillQuestionsTbl() {
		...
		if (SysData.getQuestions() != null) {
			...
			System.out.println("noQuestions In SysData");
		}
			...
			public void handle(CellEditEvent<Question, String> t) {
				...
				try {
					if (Alerts.update() == 1) {
						...
						((Question) t.getTableView().getItems().get(t.getTablePosition().getRow()))
								.setQuestion(t.getNewValue().trim());
						SysData.addQuestion(
								((Question) t.getTableView().getItems().get(t.getTablePosition().getRow())));
						SysData.Save();
					...
					final TableCell<Question, Void> cell = new TableCell<Question, Void>() {
...

						{
							...
							deleteBtn.setOnAction((ActionEvent event) -> {
								...
								try {
									if (Alerts.delete(data.getQuestion()) == 1) {
